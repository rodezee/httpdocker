    ...
    if (mg_http_match_uri(hm, "/")) { // with input
      ...
      if (mg_json_get_num(hm->body, "$[0]", &num1) &&
        ...
        if (docker) {
          ...
          responseCreate = docker_post(docker, "http://v1.25/containers/create", "{\"Image\": \"hello-world\", \"Cmd\": [\"echo -e\", \"hello world\"]}");
          if ( responseCreate == CURLE_OK )
          {
            ...
            if ( starts_with("{\"message\":\"No such image: ", dbuf) ) { // image needs to be pulled
              ...
              responsePull = docker_post(docker, "http://v1.43/images/create?fromImage=hello-world", "");
